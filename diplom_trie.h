...
#define ADD_FIRST_BYTE                            1
#define LOOKUP_FIRST_BYTE                         2
#define BYTE_SIZE                                 8
...
struct multibit_rte_lpm6_tbl_entry {
...
/** Rules tbl entry key. */
struct multibit_rte_lpm6_rule_key {
	uint8_t ip[RTE_LPM6_IPV6_ADDR_SIZE]; /**< Rule IP address. */
	uint32_t depth; /**< Rule depth. */
};

/* Header of tbl8 */
struct multibit_rte_lpm_tbl8_hdr {
...
struct multibit_rte_lpm6 {
	...
	struct multibit_rte_lpm6_tbl_entry tbl24[RTE_LPM6_TBL24_NUM_ENTRIES]
			__rte_cache_aligned; /**< LPM tbl24 table. */

	uint32_t *tbl8_pool; /**< pool of indexes of free tbl8s */
	uint32_t tbl8_pool_pos; /**< current position in the tbl8 pool */

	struct multibit_rte_lpm_tbl8_hdr *tbl8_hdrs; /* array of tbl8 headers */

	struct multibit_rte_lpm6_tbl_entry tbl8[0]
...
rule_hash(const void *data, __rte_unused uint32_t data_len,
		  uint32_t init_val)
{
	return rte_jhash(data, sizeof(struct multibit_rte_lpm6_rule_key), init_val);
}
 ...
 */
static void
tbl8_pool_init(struct multibit_rte_lpm6 *lpm)
{
	uint32_t i;
 ...
 */
static inline uint32_t
tbl8_put(struct multibit_rte_lpm6 *lpm, uint32_t tbl8_ind)
{
 ...
 */
static inline uint32_t
tbl8_available(struct multibit_rte_lpm6 *lpm)
{
 ...
 */
static inline void
rule_key_init(struct multibit_rte_lpm6_rule_key *key, uint8_t *ip, uint8_t depth)
{
 ...
 */
static void
rebuild_lpm(struct multibit_rte_lpm6 *lpm)
{
	uint64_t next_hop;
	struct multibit_rte_lpm6_rule_key *rule_key;
	uint32_t iter = 0;

	while (rte_hash_iterate(lpm->rules_tbl, (void *) &rule_key,
			(void **) &next_hop, &iter) >= 0)
		multibit_rte_lpm6_add(lpm, rule_key->ip, rule_key->depth,
			(uint32_t) next_hop);
}
#include <iostream>
/*
		...
		const struct fx_trie_config *config)
{
	...
	struct multibit_rte_lpm_tbl8_hdr *tbl8_hdrs = NULL;
	//std::cout <<"OMAR\n";

	RTE_BUILD_BUG_ON(sizeof(struct multibit_rte_lpm6_tbl_entry) != sizeof(uint32_t));
	RTE_BUILD_BUG_ON(sizeof(struct multibit_rte_lpm6_rule_key) %
		sizeof(uint32_t) != 0);
	...
	struct rte_hash_parameters rule_hash_tbl_params = {
	    ...
	    .entries = (unsigned int)(config->max_rules * 1.2) +
	    RULE_HASH_TABLE_EXTRA_SPACE,
	    .reserved = 0,
	    .key_len = sizeof(struct multibit_rte_lpm6_rule_key),
		.hash_func = rule_hash,
	...
	tbl8_hdrs = rte_malloc(NULL,
			sizeof(struct multibit_rte_lpm_tbl8_hdr) * config->number_tbl8s,
			RTE_CACHE_LINE_SIZE);
	if (tbl8_hdrs == NULL) {
...
/* Find a rule */
static inline int
rule_find_with_key(struct multibit_rte_lpm6 *lpm,
		  const struct multibit_rte_lpm6_rule_key *rule_key,
		  uint32_t *next_hop)
{
	uint64_t hash_val;
	int ret;
...
/* Find a rule */
static int
rule_find(struct multibit_rte_lpm6 *lpm, uint8_t *ip, uint8_t depth,
		  uint32_t *next_hop)
{
	struct multibit_rte_lpm6_rule_key rule_key;
 ...
 */
static inline int
rule_add(struct multibit_rte_lpm6 *lpm, uint8_t *ip, uint8_t depth, uint32_t next_hop)
{
	int ret, rule_exist;
	struct multibit_rte_lpm6_rule_key rule_key;
	uint32_t unused;
 ...
 */
static void
expand_rule(struct multibit_rte_lpm6 *lpm, uint32_t tbl8_gindex, uint8_t old_depth,
		uint8_t new_depth, uint32_t next_hop, uint8_t valid)
{
	...
	struct multibit_rte_lpm6_tbl_entry new_tbl8_entry = {
	    .next_hop = next_hop,
	    .depth = new_depth,
		.valid = valid,
		.valid_group = valid,
 ...
 */
static inline void
init_tbl8_header(struct multibit_rte_lpm6 *lpm, uint32_t tbl_ind,
		uint32_t owner_tbl_ind, uint32_t owner_entry_ind)
{
	struct multibit_rte_lpm_tbl8_hdr *tbl_hdr = &lpm->tbl8_hdrs[tbl_ind];
 ...
 */
static inline int
simulate_add_step(struct multibit_rte_lpm6 *lpm, struct multibit_rte_lpm6_tbl_entry *tbl,
		struct multibit_rte_lpm6_tbl_entry **next_tbl, const uint8_t *ip,
 ...
 */
static inline int
add_step(struct multibit_rte_lpm6 *lpm, struct multibit_rte_lpm6_tbl_entry *tbl,
		uint32_t tbl_ind, struct multibit_rte_lpm6_tbl_entry **next_tbl,
	...
	if (depth <= bits_covered) {
		...
		for (i = entry_ind; i < (entry_ind + tbl_range); i++) {
			if (!tbl[i].valid || (tbl[i].ext_entry == 0 &&
				...
				struct multibit_rte_lpm6_tbl_entry new_tbl_entry = {
					.next_hop = next_hop,
					.depth = depth,
					.valid = VALID,
					.valid_group = VALID,
...
//			std::cout << "Make new allocation"<<'\n';
		 ...
		 */
			...
			struct multibit_rte_lpm6_tbl_entry tbl_entry = {
 ...
 */
static int
simulate_add(struct multibit_rte_lpm6 *lpm, const uint8_t *masked_ip, uint8_t depth)
{
	struct multibit_rte_lpm6_tbl_entry *tbl;
	struct multibit_rte_lpm6_tbl_entry *tbl_next = NULL;
	int ret, i;
 ...
 */
template <bool branch =0>
static inline int
lookup_step(const struct multibit_rte_lpm6 *lpm, const struct multibit_rte_lpm6_tbl_entry *tbl,
		const struct multibit_rte_lpm6_tbl_entry **tbl_next, const uint8_t *ip,
		uint8_t first_byte, uint32_t *next_hop)
{
	...
	if constexpr (branch)
	{
	    ...
	    if (likely((tbl_entry & RTE_LPM6_VALID_EXT_ENTRY_BITMASK) ==
 ...
 */
int
multibit_rte_lpm6_lookup(const struct multibit_rte_lpm6 *lpm, const uint8_t *ip,
		uint32_t *next_hop)
{
	...
	tbl24_index =ip[0];// (ip[0] << BYTES2_SIZE) | (ip[1] << BYTE_SIZE) | ip[2];
				   ...
				   int32_t *next_hops, unsigned int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		do {
			 ...
			 * until success or failure
			 */
			status = lookup_step<true>(lpm, tbl, &tbl_next, ips[i],
					     first_byte++, &next_hop);
			tbl = tbl_next;
 ...
 */
static inline int
rule_delete(struct multibit_rte_lpm6 *lpm, uint8_t *ip, uint8_t depth)
{
	int ret;
	struct multibit_rte_lpm6_rule_key rule_key;
 ...
 */
static int
rule_find_less_specific(struct multibit_rte_lpm6 *lpm, uint8_t *ip, uint8_t depth,
	struct multibit_rte_lpm6_rule *rule)
{
	int ret;
	uint32_t next_hop;
	uint8_t mask;
 ...
 */
static void
rule_find_range(struct multibit_rte_lpm6 *lpm, const uint8_t *ip, uint8_t depth,
		  struct multibit_rte_lpm6_tbl_entry **from,
		  struct multibit_rte_lpm6_tbl_entry **to,
		  uint32_t *out_tbl_ind)
{
	...
	if (depth <= 8) {
		...
		*out_tbl_ind = TBL24_IND;
		...
		/* top level entry */
		struct multibit_rte_lpm6_tbl_entry *tbl = &lpm->tbl24[first_3bytes];
 ...
 */
static void
remove_tbl(struct multibit_rte_lpm6 *lpm, struct multibit_rte_lpm_tbl8_hdr *tbl_hdr,
		  uint32_t tbl_ind, struct multibit_rte_lpm6_rule *lsp_rule)
{
	struct multibit_rte_lpm6_tbl_entry *owner_entry;
	...
	if (tbl_hdr->owner_tbl_ind == TBL24_IND)
		...
		struct multibit_rte_lpm_tbl8_hdr *owner_tbl_hdr =
...
{
	uint8_t masked_ip[RTE_LPM6_IPV6_ADDR_SIZE];
	struct multibit_rte_lpm6_rule lsp_rule_obj;
	struct multibit_rte_lpm6_rule *lsp_rule;
	int ret;
	uint32_t tbl_ind;
	struct multibit_rte_lpm6_tbl_entry *from, *to;
