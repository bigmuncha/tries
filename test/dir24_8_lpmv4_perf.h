...
static struct route_rule large_route_table_ipv4[MAX_RULE_NUM];
...
#define IP_HEAD_BIT_NUM_C		3
	 ...
	 */
	start = lrand48() & mask;
	ptr_rule = &large_route_table_ipv4[num_route_entries];
...
static void insert_rule_in_random_pos(uint32_t ip, uint8_t depth)
{
	...
	tmp = large_route_table_ipv4[pos];
	large_route_table_ipv4[pos].ip = ip;
	large_route_table_ipv4[pos].depth = depth;
	if (num_route_entries < MAX_RULE_NUM)
		large_route_table_ipv4[num_route_entries++] = tmp;
}
...
static void generate_large_route_rule_table(void)
{
	...
	num_route_entries = 0;
	memset(large_route_table_ipv4, 0, sizeof(large_route_table_ipv4));
...
test_fib_perf(void)
{
	...
	print_route_distribution(large_route_table_ipv4,
	...
	for (i = 0; i < NUM_ROUTE_ENTRIES; i++) {
		if (rte_fib_add(fib, large_route_table_ipv4[i].ip,
				large_route_table_ipv4[i].depth, next_hop_add) == 0)
			status++;
	}
	/* End Timer. */
	...
	for (i = 0; i < NUM_ROUTE_ENTRIES; i++) {
		...
		status += rte_fib_delete(fib, large_route_table_ipv4[i].ip,
				large_route_table_ipv4[i].depth);
	}
